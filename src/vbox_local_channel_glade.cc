// generated 2008/2/14 2:23:17 CET by tobias@THINKPAD-T43.(none)
// using glademm V2.6.0
//
// DO NOT EDIT THIS FILE ! It was created using
// glade-- /home/tobias/src/svn/gninjam/branches/gninjam+session-mode+voice-chat/gninjam.glade
// for gtk 2.12.0 and gtkmm 2.12.0
//
// Please modify the corresponding derived classes in ./src/vbox_local_channel.cc


#if defined __GNUC__ && __GNUC__ < 3
#error This program will crash if compiled with g++ 2.x
// see the dynamic_cast bug in the gtkmm FAQ
#endif //
#include "config.h"
/*
 * Standard gettext macros.
 */
#ifdef ENABLE_NLS
#  include <libintl.h>
#  undef _
#  define _(String) dgettext (GETTEXT_PACKAGE, String)
#  ifdef gettext_noop
#    define N_(String) gettext_noop (String)
#  else
#    define N_(String) (String)
#  endif
#else
#  define textdomain(String) (String)
#  define gettext(String) (String)
#  define dgettext(Domain,Message) (Message)
#  define dcgettext(Domain,Message,Type) (Message)
#  define bindtextdomain(Domain,Directory) (Domain)
#  define _(String) (String)
#  define N_(String) (String)
#endif
#include <gtkmmconfig.h>
#if GTKMM_MAJOR_VERSION==2 && GTKMM_MINOR_VERSION>2
#include <sigc++/compatibility.h>
#define GMM_GTKMM_22_24(a,b) b
#else //gtkmm 2.2
#define GMM_GTKMM_22_24(a,b) a
#endif //
#include "vbox_local_channel_glade.hh"
#include <gtkmm/box.h>
#include <gtkmm/adjustment.h>
#include <gtkmm/label.h>
#include <gtkmm/button.h>
#include <gtkmm/separator.h>

vbox_local_channel_glade::vbox_local_channel_glade(GlademmData *gmm_data
) : Gtk::VBox(false, 4)
{  
   
   Gtk::VBox *vbox_local_channel = this;
   entry_local_channelname = Gtk::manage(new class Gtk::Entry());
   checkbutton_local_transmit = Gtk::manage(new class Gtk::CheckButton(_("T")));
   combobox_local_input = Gtk::manage(new class Gtk::ComboBox());
   checkbutton_local_stereo_in = Gtk::manage(new class Gtk::CheckButton(_("St")));
   combobox_local_output = Gtk::manage(new class Gtk::ComboBox());
   checkbutton_local_stereo_out = Gtk::manage(new class Gtk::CheckButton(_("St")));
   checkbutton_local_mute = Gtk::manage(new class Gtk::CheckButton(_("M")));
   checkbutton_local_solo = Gtk::manage(new class Gtk::CheckButton(_("S")));
   
   Gtk::HBox *hbox2 = Gtk::manage(new class Gtk::HBox(false, 4));
   Gtk::Adjustment *hscale_local_volume_adj = Gtk::manage(new class Gtk::Adjustment(0, -120, 20, 0.5, 1, 0));
   hscale_local_volume = Gtk::manage(new class Gtk::HScale(*hscale_local_volume_adj));
   
   Gtk::Adjustment *hscale_local_pan_adj = Gtk::manage(new class Gtk::Adjustment(0, -1, 1, 0.00999999977648, 0.10000000149, 0));
   hscale_local_pan = Gtk::manage(new class Gtk::HScale(*hscale_local_pan_adj));
   progressbar_local = Gtk::manage(new class Gtk::ProgressBar());
   
   Gtk::HBox *hbox3 = Gtk::manage(new class Gtk::HBox(false, 4));
   combobox_local_mode = Gtk::manage(new class Gtk::ComboBox());
   
   Gtk::Adjustment *spinbutton_bitrate_adj = Gtk::manage(new class Gtk::Adjustment(8, 8, 300, 1, 10, 10));
   spinbutton_bitrate = Gtk::manage(new class Gtk::SpinButton(*spinbutton_bitrate_adj, 1, 0));
   
   Gtk::Label *label26 = Gtk::manage(new class Gtk::Label(_("kbit/s")));
   Gtk::Label *label_filler = Gtk::manage(new class Gtk::Label(""));
   Gtk::Button *button_local_remove = Gtk::manage(new class Gtk::Button(_("Remove")));
   Gtk::HBox *hbox8 = Gtk::manage(new class Gtk::HBox(false, 4));
   Gtk::HSeparator *hseparator2 = Gtk::manage(new class Gtk::HSeparator());
   entry_local_channelname->set_name("entry_local_channelname");
   entry_local_channelname->set_flags(Gtk::CAN_FOCUS);
   entry_local_channelname->set_visibility(true);
   entry_local_channelname->set_editable(true);
   entry_local_channelname->set_max_length(0);
   entry_local_channelname->set_text("");
   entry_local_channelname->set_has_frame(true);
   entry_local_channelname->set_activates_default(false);
   checkbutton_local_transmit->set_name("checkbutton_local_transmit");
   checkbutton_local_transmit->set_flags(Gtk::CAN_FOCUS);
   _tooltips.set_tip(*checkbutton_local_transmit, _("Transmit"), "");
   checkbutton_local_transmit->set_relief(Gtk::RELIEF_NORMAL);
   checkbutton_local_transmit->set_mode(false);
   checkbutton_local_transmit->set_active(false);
   combobox_local_input->set_name("combobox_local_input");
   checkbutton_local_stereo_in->set_name("checkbutton_local_stereo_in");
   checkbutton_local_stereo_in->set_flags(Gtk::CAN_FOCUS);
   _tooltips.set_tip(*checkbutton_local_stereo_in, _("Stereo Input"), "");
   checkbutton_local_stereo_in->set_relief(Gtk::RELIEF_NORMAL);
   checkbutton_local_stereo_in->set_mode(false);
   checkbutton_local_stereo_in->set_active(false);
   combobox_local_output->set_name("combobox_local_output");
   checkbutton_local_stereo_out->set_name("checkbutton_local_stereo_out");
   checkbutton_local_stereo_out->set_flags(Gtk::CAN_FOCUS);
   _tooltips.set_tip(*checkbutton_local_stereo_out, _("Stereo Output"), "");
   checkbutton_local_stereo_out->set_relief(Gtk::RELIEF_NORMAL);
   checkbutton_local_stereo_out->set_mode(false);
   checkbutton_local_stereo_out->set_active(false);
   checkbutton_local_mute->set_name("checkbutton_local_mute");
   checkbutton_local_mute->set_flags(Gtk::CAN_FOCUS);
   _tooltips.set_tip(*checkbutton_local_mute, _("Mute"), "");
   checkbutton_local_mute->set_relief(Gtk::RELIEF_NORMAL);
   checkbutton_local_mute->set_mode(false);
   checkbutton_local_mute->set_active(false);
   checkbutton_local_solo->set_name("checkbutton_local_solo");
   checkbutton_local_solo->set_flags(Gtk::CAN_FOCUS);
   _tooltips.set_tip(*checkbutton_local_solo, _("Solo"), "");
   checkbutton_local_solo->set_relief(Gtk::RELIEF_NORMAL);
   checkbutton_local_solo->set_mode(false);
   checkbutton_local_solo->set_active(false);
   hbox2->set_name("hbox2");
   hbox2->pack_start(*entry_local_channelname);
   hbox2->pack_start(*checkbutton_local_transmit, Gtk::PACK_SHRINK, 0);
   hbox2->pack_start(*combobox_local_input, Gtk::PACK_SHRINK, 0);
   hbox2->pack_start(*checkbutton_local_stereo_in, Gtk::PACK_SHRINK, 0);
   hbox2->pack_start(*combobox_local_output, Gtk::PACK_SHRINK, 0);
   hbox2->pack_start(*checkbutton_local_stereo_out, Gtk::PACK_SHRINK, 0);
   hbox2->pack_start(*checkbutton_local_mute, Gtk::PACK_SHRINK, 0);
   hbox2->pack_start(*checkbutton_local_solo, Gtk::PACK_SHRINK, 0);
   hscale_local_volume->set_size_request(100,-1);
   hscale_local_volume->set_name("hscale_local_volume");
   hscale_local_volume->set_flags(Gtk::CAN_FOCUS);
   hscale_local_volume->set_update_policy(Gtk::UPDATE_CONTINUOUS);
   hscale_local_volume->set_inverted(false);
   hscale_local_volume->set_digits(1);
   hscale_local_volume->set_draw_value(true);
   hscale_local_volume->set_value_pos(Gtk::POS_RIGHT);
   hscale_local_pan->set_size_request(25,-1);
   hscale_local_pan->set_name("hscale_local_pan");
   hscale_local_pan->set_flags(Gtk::CAN_FOCUS);
   hscale_local_pan->set_update_policy(Gtk::UPDATE_CONTINUOUS);
   hscale_local_pan->set_inverted(false);
   hscale_local_pan->set_digits(2);
   hscale_local_pan->set_draw_value(true);
   hscale_local_pan->set_value_pos(Gtk::POS_LEFT);
   progressbar_local->set_name("progressbar_local");
   hbox3->set_name("hbox3");
   hbox3->pack_start(*hscale_local_volume);
   hbox3->pack_start(*hscale_local_pan);
   hbox3->pack_start(*progressbar_local, Gtk::PACK_SHRINK, 0);
   combobox_local_mode->set_name("combobox_local_mode");
   spinbutton_bitrate->set_name("spinbutton_bitrate");
   spinbutton_bitrate->set_flags(Gtk::CAN_FOCUS);
   _tooltips.set_tip(*spinbutton_bitrate, _("Bitrate"), "");
   spinbutton_bitrate->set_update_policy(Gtk::UPDATE_IF_VALID);
   spinbutton_bitrate->set_numeric(true);
   spinbutton_bitrate->set_digits(0);
   spinbutton_bitrate->set_wrap(false);
   label26->set_name("label26");
   label26->set_alignment(0.5,0.5);
   label26->set_padding(0,0);
   label26->set_justify(Gtk::JUSTIFY_LEFT);
   label26->set_line_wrap(false);
   label26->set_use_markup(false);
   label26->set_selectable(false);
   label_filler->set_name("label_filler");
   label_filler->set_alignment(0.5,0.5);
   label_filler->set_padding(0,0);
   label_filler->set_justify(Gtk::JUSTIFY_LEFT);
   label_filler->set_line_wrap(false);
   label_filler->set_use_markup(false);
   label_filler->set_selectable(false);
   button_local_remove->set_name("button_local_remove");
   button_local_remove->set_flags(Gtk::CAN_FOCUS);
   button_local_remove->set_relief(Gtk::RELIEF_NORMAL);
   hbox8->set_name("hbox8");
   hbox8->pack_start(*combobox_local_mode, Gtk::PACK_SHRINK, 0);
   hbox8->pack_start(*spinbutton_bitrate, Gtk::PACK_SHRINK, 0);
   hbox8->pack_start(*label26, Gtk::PACK_SHRINK, 0);
   hbox8->pack_start(*label_filler);
   hbox8->pack_start(*button_local_remove, Gtk::PACK_SHRINK, 0);
   hseparator2->set_name("hseparator2");
   vbox_local_channel->set_name("vbox_local_channel");
   vbox_local_channel->set_border_width(4);
   vbox_local_channel->pack_start(*hbox2, Gtk::PACK_SHRINK, 0);
   vbox_local_channel->pack_start(*hbox3, Gtk::PACK_SHRINK, 0);
   vbox_local_channel->pack_start(*hbox8);
   vbox_local_channel->pack_start(*hseparator2);
   entry_local_channelname->show();
   checkbutton_local_transmit->show();
   combobox_local_input->show();
   checkbutton_local_stereo_in->show();
   combobox_local_output->show();
   checkbutton_local_stereo_out->show();
   checkbutton_local_mute->show();
   checkbutton_local_solo->show();
   hbox2->show();
   hscale_local_volume->show();
   hscale_local_pan->show();
   progressbar_local->show();
   hbox3->show();
   combobox_local_mode->show();
   spinbutton_bitrate->show();
   label26->show();
   label_filler->show();
   button_local_remove->show();
   hbox8->show();
   hseparator2->show();
   vbox_local_channel->show();
   entry_local_channelname->signal_changed().connect(SigC::slot(*this, &vbox_local_channel_glade::on_entry_local_channelname_changed), false);
   checkbutton_local_transmit->signal_toggled().connect(SigC::slot(*this, &vbox_local_channel_glade::on_checkbutton_local_transmit_toggled), false);
   combobox_local_input->signal_changed().connect(SigC::slot(*this, &vbox_local_channel_glade::on_combobox_local_input_changed), false);
   checkbutton_local_stereo_in->signal_toggled().connect(SigC::slot(*this, &vbox_local_channel_glade::on_checkbutton_local_stereo_in_toggled), false);
   combobox_local_output->signal_changed().connect(SigC::slot(*this, &vbox_local_channel_glade::on_combobox_local_output_changed), false);
   checkbutton_local_stereo_out->signal_toggled().connect(SigC::slot(*this, &vbox_local_channel_glade::on_checkbutton_local_stereo_out_toggled), false);
   checkbutton_local_mute->signal_toggled().connect(SigC::slot(*this, &vbox_local_channel_glade::on_checkbutton_local_mute_toggled), false);
   checkbutton_local_solo->signal_toggled().connect(SigC::slot(*this, &vbox_local_channel_glade::on_checkbutton_local_solo_toggled), false);
   hscale_local_volume->signal_value_changed().connect(SigC::slot(*this, &vbox_local_channel_glade::on_hscale_local_volume_value_changed), false);
   hscale_local_pan->signal_value_changed().connect(SigC::slot(*this, &vbox_local_channel_glade::on_hscale_local_pan_value_changed), false);
   combobox_local_mode->signal_changed().connect(SigC::slot(*this, &vbox_local_channel_glade::on_combobox_local_mode_changed), false);
   spinbutton_bitrate->signal_value_changed().connect(SigC::slot(*this, &vbox_local_channel_glade::on_spinbutton_bitrate_value_changed), false);
   button_local_remove->signal_clicked().connect(SigC::slot(*this, &vbox_local_channel_glade::on_button_local_remove_clicked), false);
}

vbox_local_channel_glade::~vbox_local_channel_glade()
{  
}
