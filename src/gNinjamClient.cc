...
gNinjamClient::gNinjamClient(NJClient *client)
  : _client(client),
    d_connect(new class dialog_connect()),
    w_preferences(new class window_preferences())
{
  hscale_master_pan->signal_format_value().connect(SigC::slot(*this, &gNinjamClient::on_hscale_pan_format_value), false);
  hscale_metronome_pan->signal_format_value().connect(SigC::slot(*this, &gNinjamClient::on_hscale_pan_format_value), false);
  hscale_master_volume->signal_format_value().connect(SigC::slot(*this, &gNinjamClient::on_hscale_volume_format_value), false);
  hscale_metronome_volume->signal_format_value().connect(SigC::slot(*this, &gNinjamClient::on_hscale_volume_format_value), false);

  hscale_master_volume->set_value(VAL2DB(_client->config_mastervolume));
  hscale_master_pan->set_value(_client->config_masterpan);
  checkbutton_master_mute->set_active(_client->config_mastermute);
  hscale_metronome_volume->set_value(VAL2DB(_client->config_metronome));
  hscale_metronome_pan->set_value(_client->config_metronome_pan);
  checkbutton_metronome_mute->set_active(_client->config_metronome_mute);
...
{
  int status = _client->GetStatus();
  if (status >= 0) {
    if (_client->Run()) {}
    int l,p;
    _client->GetPosition(&p,&l);
  ...
  case NJClient::NJC_STATUS_OK:
    ...
    statusmsg += _client->GetHostName();
    statusmsg += " as ";
    statusmsg += _client->GetUserName();
    break;
  ...
  if (_client->GetErrorStr()[0]) {
    statusmsg += ". Server gave explanation: ";
    statusmsg += _client->GetErrorStr();
  }
...
void gNinjamClient::on_verbinden1_activate()
{
  ...
  if (response_id == Gtk::RESPONSE_OK) {
    ...
    _client->Connect(hostname.c_str(), username.c_str(), password.c_str());
  }
}
...
Glib::ustring gNinjamClient::on_hscale_volume_format_value(double value)
{
  char output[10];
...
Glib::ustring gNinjamClient::on_hscale_pan_format_value(double value)
{
  char output[9];
...
{
  _client->config_mastervolume = DB2VAL(hscale_master_volume->get_value());
}

void gNinjamClient::on_hscale_master_pan_value_changed()
{
  _client->config_masterpan = hscale_master_pan->get_value();
}

void gNinjamClient::on_hscale_metronome_volume_value_changed()
{
  _client->config_metronome = DB2VAL(hscale_metronome_volume->get_value());
}

void gNinjamClient::on_hscale_metronome_pan_value_changed()
{
  _client->config_metronome_pan = hscale_metronome_pan->get_value();
}

void gNinjamClient::on_checkbutton_master_mute_toggled()
{
  _client->config_mastermute = checkbutton_master_mute->get_active();  
}

void gNinjamClient::on_checkbutton_metronome_mute_toggled()
{
  _client->config_metronome_mute = checkbutton_metronome_mute->get_active();
}
...
void gNinjamClient::on_button_send_clicked()
{
  ...
  if (text.size() > 0) {
	...
	_client->ChatMessage_Send("MSG", text.c_str());
	...
	_client->ChatMessage_Send("ADMIN", text.c_str()+1);
      } else if (!strncasecmp(text.c_str(), "/admin ", 7)) {
	const char *p=text.c_str()+7;
	while (*p == ' ') p++;
	_client->ChatMessage_Send("ADMIN", p);
	...
	if (msg.size() > 0) {
	  _client->ChatMessage_Send("PRIVMSG", name.c_str(), msg.c_str());
	  Glib::ustring tmp;
	  tmp = "-> *";
	  tmp += name;
	  tmp += "* ";
