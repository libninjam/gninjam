...
{  
   ...
   checkbutton_mute = Gtk::manage(new class Gtk::CheckButton(_("M")));
   entry_sessionstate = Gtk::manage(new class Gtk::Entry());
   ...
   _tooltips.set_tip(*checkbutton_mute, _("Mute"), "");
   checkbutton_mute->set_relief(Gtk::RELIEF_NORMAL);
   checkbutton_mute->set_mode(false);
   checkbutton_mute->set_active(false);
   entry_sessionstate->set_name("entry_sessionstate");
   entry_sessionstate->set_flags(Gtk::CAN_FOCUS);
   _tooltips.set_tip(*entry_sessionstate, _("Maximum available session length"), "");
   entry_sessionstate->set_visibility(true);
   entry_sessionstate->set_editable(false);
   entry_sessionstate->set_max_length(0);
   entry_sessionstate->set_text("");
   entry_sessionstate->set_has_frame(true);
   entry_sessionstate->set_activates_default(false);
   entry_sessionstate->set_width_chars(17);
   ...
   hbox6->pack_start(*entry_sessionstate, Gtk::PACK_SHRINK, 0);
   vbox->set_name("vbox");
